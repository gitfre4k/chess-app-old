...
const Chessboard: React.FC = () => {
  const renderChessboard = () => {
    ...
    xyNotation.map((square, index) => {
      chessboard.push(
        <Square color="#b5915f" key={algebraicNotation[index]} id={index}></Square>
      );
    });
  };

  return <div className="chessboard">{renderChessboard1()}</div>;
};
...
const squareClickHandler = (selectedSquare: number) => {
  console.log(algebraicNotation(), xyNotation());
  if (selectedFigure) {
    setValidMoves([]);
    if (
